...
class TfrtGpuClientStreamErrorTest : public ::testing::Test {
 protected:
  void SetUp() override {
...
TEST_F(TfrtGpuClientStreamErrorTest, AbortsOnStreamError) {
  ...
  // Everything is async so we must wait for the executor to complete.
  absl::StatusOr<std::shared_ptr<xla::Literal>> result =
