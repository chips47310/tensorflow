...
// is done.
//
// Timeout determines how long we wait for the host callback to be finished
// before querying the stream status directly. It does not limit the time we
// wait for the host callback to be scheduled. If stream status is okay or
// cannot be queried, this method will wait indefinitely regardless of the
...
absl::Status BlockHostUntilDoneWithHostCallback(
    se::Stream* stream, absl::Duration timeout = absl::Seconds(5));
